...
def main(mods, datas, options):
	...
	if progress == "flag_newbie":
		...
		if len(options["inputs"]) >= 6:
			...
			datas["character"] = options[i]
			


		mods.addEchoText(options["player"],datas["character"]+" 용사님 이시군요! 신이시여,감사합니다!\n저희의 마을이 이렇게 황폐하지만 않았어도, 용사님께 귀한 대접을 해드렸을텐데, 산 아래 트롤때문에 드릴 수 있는게 없군요. 그런데 죄송하지만 용사님의 직업은 어떻게 되십니까?")
